...
public class StraightTrack implements Track {
    ...
    public StraightTrack() {
        ...
        paths[0] = new TrackPath() {
            ...
            public Location translateOffset(double offset) {
                ...
                if (doffset == 0) {
                    return StraightTrack.this.getPosition().location;
                }
                if (doffset < 0) {
                    return StraightTrack.this.computeLocation(0, doffset * -1);
                }
                if (doffset > 0) {
                    return StraightTrack.this.computeLocation(1, doffset);
                }
            ...
            public Track getTrackAt(double offset) {
                if (offset == 0) {
                    return StraightTrack.this.connection_map[0];
                }
                if (offset == length) {
                    return StraightTrack.this.connection_map[1];
                }
        ...
        paths[1] = new TrackPath() {
            ...
            public Location translateOffset(double offset) {
                ...
                if (doffset < 0) {
                    return StraightTrack.this.computeLocation(1, doffset * -1);
                }
                if (doffset > 0) {
                    return StraightTrack.this.computeLocation(0, doffset);
                }
                throw new RuntimeException(String.format("Invalid doffset {} based on offset {}.", doffset, offset));
            }
    ...
    public void disconnectTrack(int endpoint, Track track) {
        if (connection_map[endpoint] != null) {
            connection_map[endpoint].setConnector(getConnectedEndpoint(this), null);
        }
    ...
    public int getConnectedEndpoint(Track track) {
        if (track == connection_map[0]) {
            return 0;
        }
        if (track == connection_map[1]) {
            return 1;
        }
        return -1;
    }
